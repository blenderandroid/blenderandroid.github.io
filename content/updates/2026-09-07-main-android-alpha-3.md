...
summary: >
  Blender required three GPU features ARM's Mali driver does not provide, and never used two of them. Every Mali device was turned away in silence before anything reached the screen. If an earlier build refused to start, try this one.
assets:
  ...
  - name: blender-lite.apk
    label: Lite
    bytes: 202158859
    url: https://github.com/simfeo/blender/releases/download/android-alpha-3/blender-lite.apk
    sha256: 
    downloads: 7
---
...
## If an earlier build refused to start, try this one

**More devices are supported now.** Blender required three GPU features that
ARM's Mali driver does not provide, and never used two of them. Every device
with Mali graphics was turned away before anything could appear on screen, and
turned away in silence, because the reason was only written to a log a developer
would need a cable to read.

That affects a large share of non-Qualcomm phones and tablets. If a previous
release started and then closed itself, or never drew anything, this one is
worth another try.

And when something does go wrong, there is now something to send: see
...
- **Devices with ARM Mali graphics now run.** Three GPU features were required
  that ARM does not expose. Two of them, dual source blending and vertex stage
  stores, are never used by Blender at all. The third, clip distance, is used
  only by the viewport clipping region, so it costs that feature rather than the
  renderer. Tested on a Mali-G57 MC2 with ARM's r32p1 driver.
- **A startup log you can send.** Everything Blender prints while starting is
  written to `Download/blender-startup.log`, and the app shows the location when
  it opens. It is rewritten on each launch.
- When a GPU is refused, the log now names the device and the reason instead of
  reporting only that no suitable device was found.

## Reporting a problem

Attach `Download/blender-startup.log`. It records how far startup got and which
GPU was chosen, and it is written even when the app closes immediately, which is
...
- **The viewport clipping region does not work on GPUs without clip distance
  support**, which includes Mali. Nothing else depends on it.
- **Installing the full build over the lite one leaves Cycles switched off.**
  Preferences are kept across the change, and the lite build has no Cycles to
  enable, so the add-on stays disabled and the render engine does not appear.
  Turn it on in Preferences, Add-ons, or load factory settings.
- Reading and writing .blend files outside the app needs the "All files access"
  permission, which is requested on first launch.

## Tested on

Three GPU vendors, three drivers:

- Galaxy Tab S7 FE (SM-T733, Adreno 642L, Qualcomm driver, Vulkan 1.1)
- Galaxy S22 Ultra (SM-S908B, Xclipse 920, Samsung driver)
- Viewedge Tab 11 (Mediatek Helio G99 / MT6789, Mali-G57 MC2, ARM driver
  r32p1, Vulkan 1.1), reported by a user
